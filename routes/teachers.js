...
const newDb = require("../dbs/teachers.js");
const scheduleDb = require("../dbs/schedule.js");
		...
		.then(response => {
			if (response) {
				if (response.data.founded.length > 0) {
					const id = response.data.founded[0].id;
...
router.get("/teachers/my", async function(req, res, next) {
	...
	if (!errors.isEmpty()) {
		...
		try {
			...
			teachers = await getStgroupTeachers(stgroup);
			if (teachers) {
...
router.get("/teachers", function(req, res, next) {
	...
	if (!errors.isEmpty()) {
		...
		newDb
			.getTeachers(offset, limit ? limit : 10, name)
			.then(async teachers => {
				if (teachers) {
					teachers = teachers.map(async teacher => {
						const reactions = await newDb.getReactions(teacher.name);
...
router.get("/teachers/reactions", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id }
	} = req.body;
...
router.get("/teachers/comments", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id }
	} = req.body;
		...
		newDb
			...
			.then(comments => {
				comments
					...
					.map(comment => {
						if (comment.id !== parseInt(id)) comment.id = uuidv4();
					});
...
router.put("/teachers/reactions", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id },
		name,
...
router.delete("/teachers/reactions", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id }
	} = req.body;
...
router.put("/teachers/comments", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id },
		name,
...
router.delete("/teachers/comments", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id }
	} = req.body;
...
router.put("/teachers/comments/score", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id },
		name,
...
router.delete("/teachers/comments/score", function(req, res, next) {
	const errors = validationResult(req);
	const {
		params: { vk_user_id: id }
	} = req.body;
